...
GeoExt.data.PrintProvider = Ext.extend(Ext.util.Observable, {
    ...
    print: function(map, pages, options) {
        ...
        if(this.method === "GET") {
            var url = Ext.urlAppend(this.proxy ? OpenLayers.Request.makeSameOrigin(this.capabilities.printURL, this.proxy) : this.capabilities.printURL,
                "spec=" + encodeURIComponent(Ext.encode(jsonData)));
            this.download(url);
        } else {
            Ext.Ajax.request({
                url: this.proxy ? OpenLayers.Request.makeSameOrigin(this.capabilities.createURL, this.proxy) : this.capabilities.createURL,
    ...
    getAbsoluteUrl: function(url) {
        ...
        var absUrl = a.href;
        if(this.proxy){
            absUrl = OpenLayers.Request.makeSameOrigin(absUrl, this.proxy)
        }
        return absUrl;
    },
